	...
	for i in range(50, 100):
		...
		if j == 0:
			if random.choice([1, -1]) == 1:
				...
				pg.draw.ellipse(win, (0, 0, 0), (first, sec, third, fourth), random.randint(1, 2))
			else: 
				...
				pg.draw.circle(win, (0, 0, 0), center, radius, random.randint(1, 2))
		elif j == 1:
			...
			pg.draw.rect(win, (0, 0, 0), (first, sec, third, fourth), random.randint(1, 2))
		elif j == 2:
			...
			pg.draw.polygon(win, (0, 0, 0), [top, left, right], random.randint(1, 2))
		elif j == 3:
			...
			while True:
				...
				if (abs(start[0] - end[0]) >= 9 and abs(start[1] - end[1]) >=9):
					break
